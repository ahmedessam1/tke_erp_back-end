...
class EloquentCustomerRepository implements CustomerRepository
{
    ...
    {
        return Customer::GetBranchesDetails()->withYearContract()->find($customer_id);
    }
    ...
    {
        $sorting = $this->setSorting($request['sort_by'], $request['sort_type']);
        return CustomerContract::withCustomer()
            ->orderBy($sorting['sort_by'], $sorting['sort_type'])
            ->paginate(30);
    }
    ...
    public function contractStore($request)
    {
        ...
        return CustomerContract::create($fillable_values);
    }

    public function contractDelete($item_id)
    {
        $data = CustomerContract::find($item_id);
        $data->delete();
        return $data;
    }
